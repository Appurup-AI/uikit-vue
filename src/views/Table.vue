...
<template>
  ...
  <base-table :head="tableHeads" :columnTemplates="tableSizeColumns" @sorting="setSort">
    <table-row v-for="book in booksSorting" :key="book.id" :columnTemplates="tableSizeColumns">
      <table-column columnTitle="Id">{{ book.id }}</table-column>
      <table-column columnTitle="Author">{{ book.author }}</table-column>
      <table-column columnTitle="Title">{{ book.title }}</table-column>
      <table-column :image="true" :srcImage="book.image"></table-column>
      <table-column><Button label="Read Online" /></table-column> </table-row
  ></base-table>
</template>
